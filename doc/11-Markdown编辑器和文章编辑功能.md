...
### 9.1 图片上传
- 图片上传接口：`/api/v1/files/upload`（已实现）
- 支持拖拽上传和复制粘贴上传
- 上传的图片自动插入到Markdown编辑器中
- 图片URL格式：`![alt](/uploads/filename.jpg)`
...
### 10.1 图片上传方式
1. **拖拽上传**：在Markdown编辑器中直接拖拽图片文件
2. **复制粘贴**：从剪贴板粘贴图片（Ctrl+V）
3. **工具栏上传**：点击工具栏的图片按钮选择文件

### 10.2 图片自动管理
- 文章创建/更新时，系统自动从Markdown内容中提取图片URL
- 图片信息自动保存到`article_images`表
- 更新文章时，自动清理不再使用的图片记录（软删除）

### 10.3 默认封面图片
- 创建文章时自动使用默认封面图片：`/uploads/default/cover-default.jpg`
- 可以上传自定义封面图片替换默认图片
- 删除封面图片后恢复为默认图片

## 11. 后续优化建议

1. **图片管理界面**：实现图片列表、删除、统计等功能
